...
$current_date=date("Y-m-d H:i:s");
  ...
  $diff=$rowcount-1;
	$sql1 ="SELECT * FROM event ORDER BY e_id LIMIT $diff,1";
	$result1 = mysqli_query($db,$sql1);
	$sql2 ="select * from (SELECT * FROM event LIMIT 0,$diff) as Eve order by defaults";
...
// }
	...
	while($row1 = $result1->fetch_assoc()) {
		...
		$x=strpos($d,'-');
	if($x===0){
  $sql3 ="UPDATE `event` SET `default`=1 where e_id=$row1[e_id]" ;
	$result3 = mysqli_query($db,$sql3);
	}
}
?>
