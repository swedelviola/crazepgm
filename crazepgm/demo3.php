...
$totalrows=mysqli_query($db,$sql);
	...
	$result1 = mysqli_query($db,$sql1) or die($db);;
...
while($row4 = $totalrows->fetch_assoc()) {
	$dates1=date_create($current_date);
	$dates2=date_create($row4['start_date']);
	$difference1=date_diff($dates1,$dates2);
	$ds=$difference1->format('%R%i');
	$y=strpos($ds,'-');
if($y===0){
$sql6 ="UPDATE `event` SET `startdefaults`=1 where e_id=$row4[e_id]" ;
